...
def herm_linear_operator(operator: List[tn.Node]) -> sla.LinearOperator:
    ...
    num = len(operator)
    interleaved = []
    for ii, op in enumerate(operator, start=1):
        interleaved.append(op.tensor)
        interleaved.append(list(_to_uname(ax, ii) for ax in op.axis_names))
    interleaved.append(vec_shape)
    interleaved.append([str(ii) for ii in range(1, num+1)])
    interleaved.append([str(ii) for ii in range(-1, -num-1, -1)])
    expr = oe.contract_expression(*interleaved, constants=range(len(operator)))

    def matvec_(vec):
        return expr(vec.reshape(vec_shape)).reshape(-1)
